...
function makePacmen() {
  ...
  let position = randomPos(rightBoundary, bottomBoundary);
...
function changeImage(item, a, b) {
  if (item.pacImg.getAttribute("src") == `images/PacMan${a}.png`) {
    item.pacImg.setAttribute("src", `images/PacMan${b}.png`);
    return;
  }
  if (item.pacImg.getAttribute("src") == `images/PacMan${b}.png`) {
    item.pacImg.setAttribute("src", `images/PacMan${a}.png`);
    return;
  }
}
function start() {
  ...
  if (pacMen.length > 0) {
    pacMen.forEach((item) => {
      ...
      if (item.velocity.x > 0) {
        if (
          item.pacImg.getAttribute("src") === "images/PacMan3.png" ||
          item.pacImg.getAttribute("src") === "images/PacMan4.png"
        ) {
          item.pacImg.setAttribute("src", "images/PacMan1.png");
        } else {
        ...
        if (
          ...
          item.pacImg.setAttribute("src", "images/PacMan3.png");
        } else {
